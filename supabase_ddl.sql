...
(
    ...
    CONSTRAINT unique_device_user UNIQUE
(
    device_id,
    user_id
)
...
(
    ...
    CONSTRAINT user_bundle_order_user_id_fkey REFERENCES auth.users
(
    id
),
...
(
    ...
    CONSTRAINT user_bundle_order_anonymous_user_fkey REFERENCES auth.users
(
    id
),
...
(
    ...
    CONSTRAINT user_bundle_user_id_fkey REFERENCES auth.users
(
    id
),
...
(
    100
) DEFAULT 'Primary Bundle':: VARCHAR NOT NULL,
...
GRANT DELETE
, INSERT, REFERENCES, SELECT, TRUNCATE, UPDATE ON users_copy TO authenticated;
GRANT DELETE
, INSERT, REFERENCES, SELECT, TRUNCATE, UPDATE ON users_copy TO service_role;
...
GRANT DELETE
, INSERT, REFERENCES, SELECT, TRUNCATE, UPDATE ON app_config TO authenticated;
GRANT DELETE
, INSERT, REFERENCES, SELECT, TRUNCATE, UPDATE ON app_config TO service_role;
...
CREATE
OR REPLACE FUNCTION sync_users_copy()
    RETURNS TRIGGER
SECURITY DEFINER
    LANGUAGE plpgsql
    AS
...
CREATE
OR REPLACE TRIGGER trigger_sync_users_copy
    AFTER INSERT OR
UPDATE OR
DELETE
ON auth.users
...
CREATE
OR REPLACE FUNCTION get_translated_tag_by_tag_id_list(locale_param TEXT, tag_ids UUID[])
RETURNS TABLE (
    id UUID,
    tag_group_id BIGINT,
    name character varying,
