...
from app.config.context import language_context, auth_user_context, currency_context, device_id_context
...
class PromotionService:
    ...
    async def validate_promo_code(self, code: str, user_id: str, bundle: BundleDTO, device_id: str,
        ...
        if is_referral:
            ...
            referred_user: UsersCopyModel = self.__user_repo.get_by_id(record_id=user_id)
            referrer_user: UsersCopyModel = self.__user_repo.get_first_by(where={},
                                                                          filters={
            ...
            if rule.promotion_rule_action_id == PromotionRuleAction.DISCOUNT_AMOUNT.value:
                ...
                return PromotionValidationResponse(bundle=bundle,
                                                   rule_id=rule_id,
                                                   message=f"{I18n.get_message(key='DISCOUNT_PERCENTAGE', lang=lang)} {percentage} %")
            else:
                if apply_usage:
                    await self.__handle_cashback(amount=amount, beneficiary=str(rule.beneficiary),
                                                 user_id=referred_user.id, referrer_user_id=referred_user.id, code=code,
                                                 is_referral=True,
                                                 event_id=rule.promotion_rule_event_id, bundle=bundle,
            ...
            if rule.promotion_rule_action_id == PromotionRuleAction.DISCOUNT_AMOUNT.value:
                ...
                return PromotionValidationResponse(bundle=bundle,
                                                   rule_id=rule.id,
                                                   message=f"{I18n.get_message(key='DISCOUNT_AMOUNT', lang=lang)} {round(promotion.amount * rate, 2)} {currency}")
            elif rule.promotion_rule_action_id == PromotionRuleAction.DISCOUNT_PERCENTAGE.value:
                ...
                return PromotionValidationResponse(bundle=bundle,
                                                   rule_id=rule.id,
                                                   message=f"{I18n.get_message(key='DISCOUNT_PERCENTAGE', lang=lang)} {promotion.amount} %")
            else:
    ...
    def __validate_promotion(self, promotion: PromotionModel, user_id: str, device_id: str = None):
        ...
        # Validate promotion date range using date only (ignoring time)
        current_date = datetime.now(timezone.utc).date()
        valid_from_date = datetime.fromisoformat(promotion.valid_from.replace('Z', '+00:00')).date()
        valid_to_date = datetime.fromisoformat(promotion.valid_to.replace('Z', '+00:00')).date()

        if valid_from_date >= current_date or current_date >= valid_to_date:
            raise CustomException(code=404, name=ErrorMessages.PROMOTION_NOT_ACTIVE, details="promotion not active")
    ...
    async def __apply_referral_rewards(self, user_id: str, referral_code: str, paid_amount: float,
        ...
        if promotion_rule.beneficiary in [Beneficiary.REFERRED.value, Beneficiary.BOTH.value]:
            ...
            if usage is None:
                ...
                if promotion_rule.promotion_rule_action_id in [PromotionRuleAction.CASHBACK_PERCENTAGE.value,
                                                               PromotionRuleAction.CASHBACK_AMOUNT.value]:
                    ...
                    logger.info(f"No cashback for REFERRED user {user_id} as action is not cashback percentage")
        return None
