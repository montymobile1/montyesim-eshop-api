...
from app.config.config import authenticate, supabase_client
...
class AuthService:
    ...
    async def verify_otp(self, verify_otp_request: VerifyOtpRequest, device_id: str) -> Response[AuthResponseDTO]:
        try:
            logger.info(f"verify_otp request received: {verify_otp_request}")
            ...
            await self.create_wallet_if_not_exists(response.user.id)
